...
当前项目处于**开发阶段**，版本号以 `0.x.x` 开头：

- `0.x.x` 表示 API 尚未稳定，可能随时变更
- 当 API 稳定后，将发布 `1.0.0` 正式版本
...
│                      ┌─────────────────────────────┐             │
│                      │  main 分支                  │             │
│                      │  ├── feat: 添加功能 A       │ ← releasable │
│                      │  ├── fix: 修复问题 B        │ ← releasable │
│                      │  └── docs: 更新文档         │ ← 不触发     │
│                      └─────────────────────────────┘             │
│                                 │                                │
│                                 ▼                                │
│   2. Release PR 自动创建 (仅当有 releasable commits)              │
│   ────────────────────────────────────────────────               │
...
开发期间如果想合并代码但不触发发布，可以使用非 releasable 类型：
...
> **提示**: 如果开发期间不想频繁发布，可以使用 `chore` 类型提交，等准备好后再使用 `feat`/`fix` 类型触发 Release PR。
